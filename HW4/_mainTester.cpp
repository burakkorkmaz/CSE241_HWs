...
//index, value myMemory.setMem(0, 100); 
...
//Testing class CPU 
CPU myCPU(option);
myCPU.execute("MOV #0, R1", myMemory); myCPU.execute("MOV R1, #1", myMemory);
//should print in a way that similar to this:
//CPU Register Values:
//[0] -> 100
//[1] -> 0
...
myCPUProgram.ReadFile(filename); cout << myCPUProgram.getLine(0) << endl;
